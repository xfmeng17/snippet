...
// 获取队尾（最新的元素）
static TimeInfo* QueueBottom(TimeInfo* items, int head, int size, int capacity) {
  int idx = (head + size - 1) % capacity;
  return &items[idx];
}

// push，如果满了就覆盖最老的
static void QueuePush(TimeInfo* items, int* head, int* tail, int* size, int capacity,
                      const TimeInfo& info) {
  items[*tail] = info;
...
int64_t Weight::ResetWeight(size_t index, int64_t now_us) {
  ...
  if (inflight_count_ > 0) {
    ...
    int64_t punish_latency = static_cast<int64_t>(avg_latency_ * kPunishInflightRatio);
...
// 将正常/错误请求的延时采样入队
void Weight::EnqueueSample(int64_t latency, int64_t end_time_us, bool error, int64_t timeout_ms) {
  if (!error) {
    // 正常请求：新增采样点
    TimeInfo info = {latency, end_time_us};
    if (queue_size_ > 0) {
      info.latency_sum +=
          QueueBottom(queue_items_, queue_head_, queue_size_, kQueueCapacity)->latency_sum;
    }
    QueuePush(queue_items_, &queue_head_, &queue_tail_, &queue_size_, kQueueCapacity, info);
    return;
  }
  ...
  if (queue_size_ > 0) {
    TimeInfo* bottom = QueueBottom(queue_items_, queue_head_, queue_size_, kQueueCapacity);
...
bool Weight::ComputeQpsAndLatency(int64_t end_time_us, int64_t* scaled_qps) {
  TimeInfo* top = QueueTop(queue_items_, queue_head_);
  int64_t top_time_us = top->end_time_us;
  int n = queue_size_;
  *scaled_qps = kDefaultQps * kWeightScale;

  if (end_time_us > top_time_us) {
    if (n == kQueueCapacity || end_time_us >= top_time_us + 1000000L) {
      *scaled_qps =
          static_cast<int64_t>(n - 1) * 1000000L * kWeightScale / (end_time_us - top_time_us);
  ...
  return avg_latency_ != 0;
}

int64_t Weight::Update(int64_t begin_time_us, bool error, int64_t timeout_ms, size_t index) {
  ...
  EnqueueSample(latency, end_time_us, error, timeout_ms);

  int64_t scaled_qps = 0;
  if (!ComputeQpsAndLatency(end_time_us, &scaled_qps)) {
    return 0;
  }
...
Weight::AddInflightResult Weight::AddInflight(int64_t begin_time_us, size_t index, int64_t dice) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (Disabled()) {
    return {false, 0};
  }
